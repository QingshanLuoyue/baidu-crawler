...
// var searchWord = '即构';
var searchWord = '(zego | Zego | ZEGO | 即构)';
var year = 2017;
                ...
                page.includeJs("https://cdn.bootcss.com/jquery/1.11.3/jquery.min.js", function() {
                    ...
                    var content = page.evaluate(function() {
                        ...
                        var searchRes = $('#wrapper_wrapper #content_left .c-container').each(function() {
                            ...
                            var pubDate = $(this).find('.c-abstract .newTimeFactor_before_abs').text();
                            titleArr.push({
                                title: title,
                                pubDate: pubDate,
                                url: url,
                    ...
                    if (content.length !== 0) { // 获取成功，请求到数据
                        ...
                        reTryNumber++;
                        if (reTryNumber > maxTryNumber) { // 超过最大重试次数
                            // 进行下一次搜索
                            nextSearch();
                        } else {
                            console.log('获取失败，第' + reTryNumber + '次重试...')
                            setTimeout(function() {
                                startSearch(initialurl);
                            }, 1500);
                        }
...
// 暂存获取到的百度搜索结果数据
var historyFilePath = './record.txt';
// 设置记录历史请求记录
function setRecordHistory(count, word, writedNumber) {
    var str = '';
    str += '{"count":' + count + ',"word":"' + word + '","writedNumber":' + writedNumber + '}';
    fs.write('./record.txt', str, 'w');
}

// 获取记录历史请求记录
function getRecordHistory() {
    var readTxt = fs.open('./record.txt', 'r');
